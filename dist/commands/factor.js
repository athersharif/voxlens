...
var _DefaultExportValue = function _DefaultExportValue(data, options) {
  var formattedData = Array.isArray(data.x[0]) ? data.x : [data.x];
  var xs = Array.isArray(options.x) ? options.x : [options.x];
  var sentence = '';
  xs.forEach(function (x, i) {
    if (!options.key || options.key && x === options.key) {
      var values = _get__("uniq")(formattedData[i]);
      sentence += 'Data is from ' + values.length + ' ' + _get__("pluralize")(x);
      if (options.listAll) {
        sentence += ': ' + values.join(', ');
      } else {
        sentence += '. Say tell me factor levels for ' + x + ' to hear all ' + _get__("pluralize")(x);
      }
      sentence += '. ';
    }
  });
